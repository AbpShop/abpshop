                ...
                <Row type="flex" :gutter="24">
                    <Col v-bind="grid">
                        <FormItem label="搜索："  label-for="keyword">
                            <Input search enter-button  v-model="levelFrom.keyword" placeholder="请输入物流公司名称或者编码" @on-search="userSearchs"/>
                        </FormItem>
                    </Col>
                </Row>
                <Row type="flex">
                    <Col v-bind="grid">
                        <Button type="primary"  icon="md-add" @click="add">添加物流公司</Button>
                    </Col>
                </Row>
            </Form>
...
<script>
    ...
    import { freightCreateApi, freightListApi, freightEditApi, freightStatusApi } from '@/api/setting';
    export default {
        name: 'user_level',
        data () {
            return {
        ...
        methods: {
            ...
            onchangeIsShow (row) {
                ...
                freightStatusApi(data).then(async res => {
                    this.$Message.success(res.msg);
                }).catch(res => {
                    this.$Message.error(res.msg);
                })
            ...
            getList () {
                this.loading = true;
                freightListApi(this.levelFrom).then(async res => {
                    let data = res.data
