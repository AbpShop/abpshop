<template>
	<view class="explosion" :style="'margin-top:' + mbConfig +'rpx;background-color:' + boxColor+';'" v-if="explosiveMoney.length">
		<view class="hd">
			<!-- <image src="/static/images/explosion-title.png" mode=""></image> -->
			<view class="title" :style="'color:'+themeColor+';'">{{titleConfig}}</view>
			<view class="txt" :style="'background: linear-gradient(90deg, '+ bgColor[0].item +' 0%, '+ bgColor[1].item +' 100%);'">{{desConfig}}</view>
		</view>
		<view class="bd">
			<navigator class="item" v-for="(item,index) in explosiveMoney" :key="index" :url="item.info[2].value"
			 hover-class='none'>
				<view class="con-box">
