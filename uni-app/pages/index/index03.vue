<template>
	<view class="page-index copy-data" :class="{'bgf':navIndex >0}">
		<!-- #ifdef H5 -->
		<view class="header">
		...
		<view class="page_content" :style="'margin-top:'+(marTop+30)+'px;'" v-if="navIndex == 0">
			...
			<!-- #endif -->
			<!-- 拼团热点图 -->
			<view class="hot-img" :style="{'marginTop':liveList.length>0?'40rpx':'0'}" v-if="activityList.length">
			...
			<!-- 超值爆款 -->
			<view class="explosion" v-if="explosiveMoney.length">
				<view class="hd">
			...
			<!-- 精品推荐 -->
			<view class='boutique' v-if="bastBanner.length">
		...
		methods: {
			newCoupon() {
				getNewCoupon().then(res => {
					...
					setTimeout(() => {
						if (data.list.length) {
							this.window = data.show ? true : false;
						} else {
							this.window = false
						}
					}, 300)
