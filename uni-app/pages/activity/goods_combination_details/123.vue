<template>
	<view>
		...
		<!-- #endif -->
		<home></home>
		<product-window :attr='attribute' @myevent="onMyEvent" @ChangeAttr="ChangeAttr" @ChangeCartNum="ChangeCartNum"
...
<script>
	...
	export default {
		...
		methods: {
			...
			attrVal(val) {
				console.log(val, 'attrVal')
				this.attribute.productAttr[val.indexw].index = val.indexn;
			},
			/**
			 * 属性变动赋值
			 * 
			...
			goCat() {
				...
				//如果有属性,没有选择,提示用户选择
				if (this.attribute.productAttr.length && productSelect === undefined && this.isOpen == true) return app.Tips({
					title: '请选择属性'
				});
				var data = {
				...
				postCartAdd(data).then(function(res) {
					...
					wx.navigateTo({
						url: '/pages/users/order_confirm/index?cartId=' + res.cartId
					});
				}).catch(function(res) {
					uni.showToast({
						title: res,
